<x-admin-layout title="Edit Seller">
    <div class="max-w-2xl mx-auto">
        <div class="rounded-card border border-gray-100 bg-white p-8 shadow-card">
                <form method="POST" action="{{ route('admin-legacy.sellers.update', $seller) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                                  class="mt-2 w-full rounded-lg border border-slate-300 px-4 py-2 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-200">{{ old('description', $seller->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <a href="{{ route('admin-legacy.sellers.show', $seller) }}" class="rounded-lg border border-slate-300 px-6 py-2 font-semibold text-slate-900 hover:bg-slate-50">
                            Cancel
                        </a>
                    </div>
                </form>
