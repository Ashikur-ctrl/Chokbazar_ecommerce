<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-sm font-semibold text-brand-600">Order tracking</p>
    ...
    <div class="bg-slate-50 py-10">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-brand-100">
                <div class="space-y-5">
                    @foreach($steps as $status => $label)
                        @php($done = $currentIndex !== false && $loop->index <= $currentIndex)
                        <div class="flex gap-4">
                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full {{ $done ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-400' }}">{{ $loop->iteration }}</div>
                            <div>
                ...
                @if($order->tracking_number)
                    <div class="mt-6 rounded-md bg-brand-50 p-4 text-sm text-slate-700">
