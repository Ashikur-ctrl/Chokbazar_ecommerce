    ...
    <form method="POST" action="{{ route('seller.products.update', $product) }}" enctype="multipart/form-data" class="max-w-3xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-4 gap-3">
                        @foreach($product->images as $image)
                            <div class="relative aspect-square rounded-lg overflow-hidden bg-gray-100">
                                ...
                                @if($image->is_primary)
                                    <span class="absolute top-1 left-1 bg-orange-500 text-white text-xs px-1.5 py-0.5 rounded">Primary</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                <button type="submit" class="rounded-lg bg-orange-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-orange-700">Update Product</button>
